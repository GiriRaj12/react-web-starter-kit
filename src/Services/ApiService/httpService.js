...
export function get(params, extensionUrl) {
  return fetch(getFormattedURL(params, extensionUrl)).then((res) => res.json());
}
...
export function post(payload, extensionUrl) {
  return fetchAPI(
    getFormattedURL(null, extensionUrl),
    formHeadder(POST, payload)
  );
}
...
export function put(params, payload, extensionUrl) {
  return fetch(
    getFormattedURL(params, extensionUrl),
    formHeadder(PUT, payload)
  ).then((res) => res.json());
}
  ...
  if (params) {
    ...
    for (let key in params) {
      if (i === 0) {
        ...
        i++;
      } else formattedURL = formattedURL + "&" + key + "=" + params[key];
    }
  }
  return formattedURL;
}
